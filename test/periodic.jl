...
context("Periodic Boundary:")do
    ...
    approx_order = 2
    x = collect(1:1.0:N).^2
    A = DiffEqLinearOperator{Float64}(2,2,1.0,N,:periodic,:periodic)
    ...
    A = DiffEqLinearOperator{Float64}(d_order,approx_order,1.0,N,:periodic,:periodic)
    boundary_points = A.boundary_point_count

    res = A*y
    @test res[boundary_points[1] + 1: N - boundary_points[2]] ≈ 24.0*ones(N - sum(boundary_points)) atol=10.0^-1; # Float64 is less stable

    A = DiffEqLinearOperator{BigFloat}(d_order,approx_order,one(BigFloat),N,:periodic,:periodic)
