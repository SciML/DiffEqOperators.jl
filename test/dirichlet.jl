...
context("Dirichlet0 Boundary Conditions:")do
    ...
    A = LinearOperator{Float64}(2,2,1/99,length(x),:Dirichlet0,:Dirichlet0,bndry_fn=(0.0,0.0))
    ...
    A = LinearOperator{Float64}(d_order,approx_order,1.0,N,:Dirichlet0,:Dirichlet0,bndry_fn=(0.0,0.0))
    boundary_points = A.boundary_point_count
    res = A*y
    @test res[boundary_points + 1: N - boundary_points] ≈ 24.0*ones(N - 2*boundary_points) atol=10.0^-1; # Float64 is less stable

    A = LinearOperator{BigFloat}(d_order,approx_order,one(BigFloat),N,:Dirichlet0,:Dirichlet0,bndry_fn=(0.0,0.0))
