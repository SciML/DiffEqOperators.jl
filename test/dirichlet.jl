...
context("Dirichlet Boundary Conditions: ")do
    ...
    A = LinearOperator{Float64}(2,2,1/99,length(x),:D0,:D0,bndry_fn=(0.0,0.0))
    ...
    A = LinearOperator{Float64}(d_order,approx_order,1.0,N,:D0,:D0,bndry_fn=(0.0,0.0))
    boundary_points = A.boundary_point_count
    res = A*y
    @test res[boundary_points + 1: N - boundary_points] ≈ 24.0*ones(N - 2*boundary_points) atol=10.0^-1; # Float64 is less stable

    A = LinearOperator{BigFloat}(d_order,approx_order,one(BigFloat),N,:D0,:D0,bndry_fn=(0.0,0.0))
