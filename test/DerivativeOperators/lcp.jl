...
# Setup
StoppingProblem = @with_kw (
    μ_bar = -0.01, # 1D Brownian motion with drift
    σ_bar = 0.01,
    S_bar = 10.0,
    γ = 0.5, # u(x) = x^γ
...
function LCPsolve(sp)
    ...
    f = z -> ρ .* z - L1 * z - L2 * z - u + ρ .* S - L1 * S - L2 * S
    n = sp.M
    lb = zeros(n)
    ub = 300 .* ones(n) # a reasonable guess
    options(convergence_tolerance = 1e-15, output = :no, time_limit = 600) # 10 minute budget
