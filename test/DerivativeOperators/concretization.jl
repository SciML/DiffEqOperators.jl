...
@testset "First Dimension" begin
    ...
    @test L1 * M ≈
          Array(L1, size(M)) * vec(M) ≈
          sparse(L1, size(M)) * M ≈
          BandedMatrix(L1, size(M)) * M ≈
          BandedBlockBandedMatrix(L1, size(M)) * M
    @test L2 * M ≈
          Array(L2, size(M)) * vec(M) ≈
          sparse(L2, size(M)) * M ≈
          BandedMatrix(L2, size(M)) * M ≈
          BandedBlockBandedMatrix(L2, size(M)) * M
    @test L3 * M ≈
          Array(L3, size(M)) * vec(M) ≈
          sparse(L3, size(M)) * M ≈
          BandedMatrix(L3, size(M)) * M ≈
          BandedBlockBandedMatrix(L3, size(M)) * M

    M = rand(22, 2, 2, 2, 2)

    @test vec(L1 * M) ≈
          Array(L1, size(M)) * vec(M) ≈
          sparse(L1, size(M)) * vec(M) ≈
          BandedMatrix(L1, size(M)) * vec(M) ≈
          BandedBlockBandedMatrix(L1, size(M)) * vec(M)
    @test vec(L2 * M) ≈
          Array(L2, size(M)) * vec(M) ≈
          sparse(L2, size(M)) * vec(M) ≈
          BandedMatrix(L2, size(M)) * vec(M) ≈
          BandedBlockBandedMatrix(L2, size(M)) * vec(M)
    @test vec(L3 * M) ≈
          Array(L3, size(M)) * vec(M) ≈
          sparse(L3, size(M)) * vec(M) ≈
          BandedMatrix(L3, size(M)) * vec(M) ≈
          BandedBlockBandedMatrix(L3, size(M)) * vec(M)
...
@testset "Biased Upwind : Positive Wind with offside, regular grid" begin
    ...
    @test L1 * M ≈
          ...
          BandedBlockBandedMatrix(L1, size(M)) * M ≈
          K1 * M ≈
          Array(K1, size(M)) * vec(M) ≈
          sparse(K1, size(M)) * M ≈
          BandedMatrix(K1, size(M)) * M ≈
          BandedBlockBandedMatrix(K1, size(M)) * M
    @test L2 * M ≈
          Array(L2, size(M)) * vec(M) ≈
          sparse(L2, size(M)) * M ≈
          BandedMatrix(L2, size(M)) * M ≈
          BandedBlockBandedMatrix(L2, size(M)) * M ≈
          K2 * M ≈
          Array(K2, size(M)) * vec(M) ≈
          sparse(K2, size(M)) * M ≈
          BandedMatrix(K2, size(M)) * M ≈
          BandedBlockBandedMatrix(K2, size(M)) * M
    @test L2 * P ≈
          Array(L2, size(P)) * vec(P) ≈
          sparse(L2, size(P)) * P ≈
          BandedMatrix(L2, size(P)) * P ≈
          BandedBlockBandedMatrix(L2, size(P)) * P ≈
          K2 * P ≈
          Array(K2, size(P)) * vec(P) ≈
          sparse(K2, size(P)) * P ≈
          BandedMatrix(K2, size(P)) * P ≈
          BandedBlockBandedMatrix(K2, size(P)) * P
    @test L3 * M ≈
          Array(L3, size(M)) * vec(M) ≈
          sparse(L3, size(M)) * M ≈
          BandedMatrix(L3, size(M)) * M ≈
          BandedBlockBandedMatrix(L3, size(M)) * M ≈
          K3 * M ≈
          Array(K3, size(M)) * vec(M) ≈
          sparse(K3, size(M)) * M ≈
          BandedMatrix(K3, size(M)) * M ≈
          BandedBlockBandedMatrix(K3, size(M)) * M
    @test L4 * M ≈
          Array(L4, size(M)) * vec(M) ≈
          sparse(L4, size(M)) * M ≈
          BandedMatrix(L4, size(M)) * M ≈
          BandedBlockBandedMatrix(L4, size(M)) * M ≈
          K4 * M ≈
          Array(K4, size(M)) * vec(M) ≈
          sparse(K4, size(M)) * M ≈
          BandedMatrix(K4, size(M)) * M ≈
          BandedBlockBandedMatrix(K4, size(M)) * M
    @test L5 * M ≈
          Array(L5, size(M)) * vec(M) ≈
          sparse(L5, size(M)) * M ≈
          BandedMatrix(L5, size(M)) * M ≈
          BandedBlockBandedMatrix(L5, size(M)) * M ≈
          K5 * M ≈
          Array(K5, size(M)) * vec(M) ≈
          sparse(K5, size(M)) * M ≈
          BandedMatrix(K5, size(M)) * M ≈
          BandedBlockBandedMatrix(K5, size(M)) * M
    ...
    @test vec(L1 * M) ≈
          ...
          vec(K1 * M) ≈
          Array(K1, size(M)) * vec(M) ≈
          sparse(K1, size(M)) * vec(M) ≈
          BandedMatrix(K1, size(M)) * vec(M) ≈
          BandedBlockBandedMatrix(K1, size(M)) * vec(M)
    @test vec(L2 * M) ≈
          Array(L2, size(M)) * vec(M) ≈
          sparse(L2, size(M)) * vec(M) ≈
          BandedMatrix(L2, size(M)) * vec(M) ≈
          ...
          vec(K2 * M) ≈
          Array(K2, size(M)) * vec(M) ≈
          sparse(K2, size(M)) * vec(M) ≈
          BandedMatrix(K2, size(M)) * vec(M) ≈
          BandedBlockBandedMatrix(K2, size(M)) * vec(M)
    @test vec(L3 * M) ≈
          Array(L3, size(M)) * vec(M) ≈
          sparse(L3, size(M)) * vec(M) ≈
          BandedMatrix(L3, size(M)) * vec(M) ≈
          ...
          vec(K3 * M) ≈
          Array(K3, size(M)) * vec(M) ≈
          sparse(K3, size(M)) * vec(M) ≈
          BandedMatrix(K3, size(M)) * vec(M) ≈
          BandedBlockBandedMatrix(K3, size(M)) * vec(M)
    @test vec(L4 * M) ≈
          Array(L4, size(M)) * vec(M) ≈
          sparse(L4, size(M)) * vec(M) ≈
          BandedMatrix(L4, size(M)) * vec(M) ≈
          BandedBlockBandedMatrix(L4, size(M)) * vec(M) ≈
          vec(K4 * M) ≈
          Array(K4, size(M)) * vec(M) ≈
          sparse(K4, size(M)) * vec(M) ≈
          BandedMatrix(K4, size(M)) * vec(M) ≈
          BandedBlockBandedMatrix(K4, size(M)) * vec(M)
    @test vec(L5 * M) ≈
          Array(L5, size(M)) * vec(M) ≈
          sparse(L5, size(M)) * vec(M) ≈
          BandedMatrix(L5, size(M)) * vec(M) ≈
          BandedBlockBandedMatrix(L5, size(M)) * vec(M) ≈
          vec(K5 * M) ≈
          Array(K5, size(M)) * vec(M) ≈
          sparse(K5, size(M)) * vec(M) ≈
          BandedMatrix(K5, size(M)) * vec(M) ≈
          BandedBlockBandedMatrix(K5, size(M)) * vec(M)
...
@testset "BC concretizations" begin
    ...
    bc4 = RobinBC(
        (params[1], -params[2], left_RBC),
        (params[1], params[2], right_RBC),
        1.0,
        1,
    )
    bc5 = GeneralBC(
        [-left_RBC, params[1], -params[2]],
        [-right_RBC, params[1], params[2]],
        1.0,
        1,
    )
