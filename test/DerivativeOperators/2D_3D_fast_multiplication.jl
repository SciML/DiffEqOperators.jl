...
@testset "2D Multiplication with no boundary points and dx = 1.0" begin
    ...
    @test M_temp ≈ (
        (Lx*M)[1:N, 2:N+1] + (Ly*M)[2:N+1, 1:N] + (Lxx*M)[1:N, 2:N+1] + (Lyy*M)[2:N+1, 1:N]
    )
end
...
@testset "3D Multiplication with no boundary points and dx = dy = dz = 1.0" begin
    ...
    @test M_temp ≈ (
        (Lxx*M)[1:N, 2:N+1, 2:N+1] +
        (Lyy*M)[2:N+1, 1:N, 2:N+1] +
    ...
    @test M_temp ≈ (
        (Lx*M)[1:N, 2:N+1, 2:N+1] +
        (Ly*M)[2:N+1, 1:N, 2:N+1] +
        (Lxx*M)[1:N, 2:N+1, 2:N+1] +
        (Lyy*M)[2:N+1, 1:N, 2:N+1] +
        (Lz*M)[2:N+1, 2:N+1, 1:N] +
        (Lzz*M)[2:N+1, 2:N+1, 1:N]
    )
...
@testset "3D Multiplication with identical bpc and dx = dy = dz = 1.0" begin
    ...
    @test M_temp ≈ (
        (Lx4*M)[1:N, 2:N+1, 2:N+1] +
        (Ly4*M)[2:N+1, 1:N, 2:N+1] +
...
@testset "3D Multiplication with differing bpc and dx = dy = dz = 1.0" begin
    ...
    @test M_temp ≈
          (
        (Lx2*M)[1:N, 2:N+1, 2:N+1] +
        (Ly2*M)[2:N+1, 1:N, 2:N+1] +
        (Lz2*M)[2:N+1, 2:N+1, 1:N] +
        (Lx3*M)[1:N, 2:N+1, 2:N+1] +
        (Ly3*M)[2:N+1, 1:N, 2:N+1] +
        (Lz3*M)[2:N+1, 2:N+1, 1:N] +
        (Lx4*M)[1:N, 2:N+1, 2:N+1] +
...
@testset "irregular x grid and regular y and z grids (dy = 0.25, dz = 0.05)" begin
    ...
    @test M_temp ≈ (
        ...
        (Ly4*M)[2:N+1, 1:N, 2:N+1] +
        (Lz4*M)[2:N+1, 2:N+1, 1:N] +
        (_Ly2*M)[2:N+1, 1:N, 2:N+1] +
        (_Lz3*M)[2:N+1, 2:N+1, 1:N] +
...
@testset "irregular y grid and regular x and z gris (dx = 0.25, dz = 0.05)" begin
    ...
    mul!(M_temp, A, M)
    @test M_temp ≈
          ((Ly3*M)[:, 1:N, 2:N+1] + (Lz2*M)[:, 2:N+1, 1:N] + (Lz3*M)[:, 2:N+1, 1:N])
    ...
    mul!(M_temp, A, M)
    @test M_temp ≈
          ((Ly2*M)[:, 1:N, 2:N+1] + (Lz2*M)[:, 2:N+1, 1:N] + (Lz3*M)[:, 2:N+1, 1:N])
