...
for dimensionality = 2:5
    for dim = 1:dimensionality
        ...
        Apad = DiffEqOperators.BoundaryPaddedArray{
            Float64,
            dim,
            dimensionality,
            dimensionality - 1,
            ...
            lower,
            upper,
            selectdim(A, dim, 2:(size(A)[dim]-1)),
        )
...
for i = 1:n, j = 1:m #test getindex for all indicies of Apad
    @test A[i, j] == Apad[i, j]
end
...
upper = Matrix[
    A[end, 2:(end-1), 2:(end-1)],
    A[2:(end-1), end, 2:(end-1)],
