...
struct ImplicitFiniteDifference{T} <: DiffEqBase.AbstractDiscretization
    dxs::T
    upwind_order::Int
    centered_order::Int
...
struct MOLFiniteDifference{T,T2} <: DiffEqBase.AbstractDiscretization
    space::ImplicitFiniteDifference{T}
    time::T2
end

# Constructors. If no order is specified, both upwind and centered differences will be 2nd order
MOLFiniteDifference(dxs, time; upwind_order = 1, centered_order = 2, grid_align=center_align) =
    MOLFiniteDifference(ImplicitFiniteDifference(dxs, upwind_order, centered_order, grid_align), time)
...
function SciMLBase.symbolic_discretize(pdesys::ModelingToolkit.NonlinearSystem,discretization::DiffEqOperators.ImplicitFiniteDifference)
    grid_align = discretization.grid_align
    pdeeqs = pdesys.eqs isa Vector ? pdesys.eqs : [pdesys.eqs]
    nottime = filter(x->~isequal(x.val),pdesys.indvars)
    nspace = length(nottime)
    ...
    # Get tspan
    tdomain = pdesys.domain[findfirst(d->isequal(d.variables),pdesys.domain)]
    @assert tdomain.domain isa IntervalDomain
    tspan = (tdomain.domain.lower,tdomain.domain.upper)
...
function SciMLBase.discretize(nonlinsys::ModelingToolkit.NonlinearSystem,discretization::DiffEqOperators.ImplicitFiniteDifference)
    sys, tspan = SciMLBase.symbolic_discretize(nonlinsys,discretization)
    simpsys = structural_simplify(sys)
    prob = NonlinearProblem(simpsys,Pair[])
end
