...
function convolve_interior!(
    ...
    if eltype(stencil) <: AbstractVector
        @turbo for i = (1+A.boundary_point_count):(length(x_temp)-A.boundary_point_count)
            xtempi = zero(T)
...
for MT in [2, 3]
    @eval begin
        function LinearAlgebra.mul!(
            x_temp::AbstractArray,
            has_vector::Bool,
            A::Union{GradientOperator{T,$MT},DivergenceOperator{T,$MT}},
            M::AbstractArray;
            overwrite = false,
        ) where {T}
...
function convolve_interior!(
    x_temp::AbstractArray{T1},
    A::GradientOperator{T2,2},
    x::AbstractArray{T3};
    overwrite = false,
) where {T1,T2,T3}
...
function convolve_BC_left!(
    ...
    # Along 1st axis
    @turbo for j = 1:s[2], i = 1:bpc
        cur_stencil_1 = stencil_1[i]
        cur_coeff_1 = coeff_1[i]
        x_temp1 = zero(T)
        for idx = 1:(A.ops[1].boundary_stencil_length)
            x_temp1 += cur_coeff_1 * cur_stencil_1[idx] * x[idx, j]
        end
        x_temp[i, j, 1] += x_temp1
    end
    # Along 2nd axis
    @turbo for j = 1:bpc, i = 1:s[1]
        cur_stencil_2 = stencil_2[j]
        cur_coeff_2 = coeff_2[j]
        x_temp2 = zero(T)
        for idx = 1:(A.ops[2].boundary_stencil_length)
...
function convolve_BC_right!(
    ...
    # Compute derivatives along particular axis and aggregate the outputs
    # Along 1st axis
    @turbo for j = 1:s[2], i = s[1]-bpc+1:s[1]
    ...
    # Along 2nd axis
    @turbo for j = s[2]-bpc+1:s[2], i = 1:s[1]
...
function convolve_interior!(
    ...
    if eltype(stencil_1) <: AbstractArray
        # Along 1st axis
        @turbo for k = 1:s[3], j = 1:s[2], i = 1+bpc:s[1]-bpc
            cur_stencil_1 = stencil_1[i-bpc]
            cur_coeff_1 = coeff_1[i]
            x_temp1 = zero(T)
            for idx = 1:(A.ops[1].stencil_length)
        ...
        # Along 2nd axis
        @turbo for k = 1:s[3], j = 1+bpc:s[2]-bpc, i = 1:s[1]
            cur_stencil_2 = stencil_2[j-bpc]
            cur_coeff_2 = coeff_2[j]
            x_temp2 = zero(T)
            for idx = 1:(A.ops[2].stencil_length)
        ...
        # Along 3rd axis
        @turbo for k = 1+bpc:s[3]-bpc, j = 1:s[2], i = 1:s[1]
            cur_stencil_3 = stencil_3[k-bpc]
            cur_coeff_3 = coeff_3[k]
            x_temp3 = zero(T)
            for idx = 1:(A.ops[3].stencil_length)
...
function convolve_BC_left!(
    ...
    # Along 1st axis
    @turbo for k = 1:s[3], j = 1:s[2], i = 1:bpc
        cur_stencil_1 = stencil_1[i]
        cur_coeff_1 = coeff_1[i]
        x_temp1 = zero(T)
        for idx = 1:(A.ops[1].boundary_stencil_length)
    ...
    # Along 2nd axis
    @turbo for k = 1:s[3], j = 1:bpc, i = 1:s[1]
        cur_stencil_2 = stencil_2[j]
        cur_coeff_2 = coeff_2[j]
        x_temp2 = zero(T)
        for idx = 1:(A.ops[2].boundary_stencil_length)
    ...
    # Along 3rd axis
    @turbo for k = 1:bpc, j = 1:s[2], i = 1:s[1]
        cur_stencil_3 = stencil_3[k]
        cur_coeff_3 = coeff_3[k]
        x_temp3 = zero(T)
        for idx = 1:(A.ops[3].boundary_stencil_length)
...
function convolve_BC_right!(
    ...
    # Compute derivatives along particular axis and aggregate the outputs
    # Along 1st axis
    @turbo for k = 1:s[3], j = 1:s[2], i = s[1]-bpc+1:s[1]
        cur_stencil_1 = stencil_1[i-s[1]+bpc]
        cur_coeff_1 = coeff_1[i]
        x_temp1 = zero(T)
        for idx = 1:bstl
    ...
    # Along 2nd axis
    @turbo for k = 1:s[3], j = s[2]-bpc+1:s[2], i = 1:s[1]
        cur_stencil_2 = stencil_2[j-s[2]+bpc]
        cur_coeff_2 = coeff_2[j]
        x_temp2 = zero(T)
        for idx = 1:bstl
    ...
    # Along 3rd axis
    @turbo for k = s[3]-bpc+1:s[3], j = 1:s[2], i = 1:s[1]
