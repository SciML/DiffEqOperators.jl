...
function dot_product!(
    u::AbstractArray{T1,N},
    A::AbstractArray{T2,N2},
    B::AbstractArray{T3,N2},
) where {T1<:Number,T2<:Number,T3<:Number,N,N2}

    (size(A) === size(B)) || throw(ArgumentError("Vectors must have the same shape"))
    (N === N2 - 1) || throw(
        ArgumentError("Output should be scalar matrix, one dimension less than input"),
    )
...
function cross_product(
    A::AbstractArray{T1,4},
    B::AbstractArray{T2,4},
...
function cross_product!(
    u::AbstractArray{T1,4},
    A::AbstractArray{T2,4},
    B::AbstractArray{T3,4},
    ...
    @turbo for r = 1:s[3], q = 1:s[2], p = 1:s[1]
...
function square_norm!(
    u::AbstractArray{T1,N1},
    A::AbstractArray{T2,N2},
) where {T1<:Number,T2<:Number,N1,N2}
    dot_product!(u, A, A)
    @. u = u^0.5
end
