...
function Base.copyto!(L::AbstractMatrix{T}, A::DerivativeOperator{T}, N::Int) where {T}
    ...
    for i = 1:bl
        ...
        cur_stencil =
            use_winding(A) && cur_coeff < 0 ? reverse(A.low_boundary_coefs[i]) :
    ...
    for i = bl+1:N-bl
        cur_coeff = get_coeff(i)
        stencil =
            eltype(A.stencil_coefs) <: AbstractVector ? A.stencil_coefs[i-bl] :
            A.stencil_coefs
    ...
    for i = N-bl+1:N
        cur_coeff = get_coeff(i)
        cur_stencil =
            use_winding(A) && cur_coeff < 0 ? reverse(A.high_boundary_coefs[i-N+bl]) :
...
function SparseArrays.SparseMatrixCSC(A::DerivativeOperator{T}, N::Int = A.len) where {T}
    ...
    for i = bl+1:N-bl
        ...
        stencil =
            ...
            A.stencil_coefs
        cur_stencil = use_winding(A) && cur_coeff < 0 ? reverse(stencil) : stencil
        append!(Is, ((i for j = 1:stencil_length)...))
    ...
    for i = N-bl+1:N
        ...
        cur_stencil =
            use_winding(A) && cur_coeff < 0 ? reverse(A.high_boundary_coefs[i-N+bl]) :
            A.high_boundary_coefs[i-N+bl]
        append!(Is, ((i for j = N-bstl+3:N+2)...))
...
) where {Mat<:Union{Array,SparseMatrixCSC,BandedMatrix}} = Mat(A)
...
function LinearAlgebra.Array(
    Q::MultiDimDirectionalBC{T,B,D,N,L},
    s::NTuple{N,G},
) where {T,B,D,N,L,G<:Int}
    ...
    ranges = Union{typeof(1:10),Int64}[1:s[i] for i = 1:N]
    ranges[D] = ranges[D] .+ 1
...
function LinearAlgebra.Array(
    ...
    ranges = Union{typeof(1:10),Int64}[2:s_pad[i]-1 for i = 1:N] #Set up indices corresponding to the interior
    interior = CartesianIndices(Tuple(ranges))
    ...
    for dim = 1:N #Loop over boundaries
        r_ = deepcopy(ranges)
        r_[dim] = 1
        ...
        for K in CartesianIndices(upper) #for every element of the boundaries
            ...
            for k = 1:s[dim] #loop over the direction orthogonal to the boundary
...
function BlockBandedMatrices.BandedBlockBandedMatrix(
    A::DerivativeOperator{T,N},
    Mshape,
) where {T,N}
    # Case where A is not differentiating along the first dimension
    if N != 1
...
function LinearAlgebra.Array(A::DerivativeOperator{T,N,true}, len::Int = A.len) where {T,N}
    ...
    for i = bpc+1:len-bpc-offside
        cur_coeff = coeff[i]
        cur_stencil = stencils
        cur_stencil =
            cur_coeff >= 0 ? cur_stencil : ((-1)^A.derivative_order) * reverse(cur_stencil)
        if cur_coeff >= 0
            L[i, i+1-offside:i+stl-offside] = cur_coeff * cur_stencil
        else
    ...
    for i = 1:bpc+offside
        ...
        elseif cur_coeff >= 0 && i < offside + 1
            cur_stencil = stencils
            L[len-bpc+i-offside, len-stl+i-offside+3:len+i-offside+2] =
                cur_coeff * cur_stencil
        else
...
function LinearAlgebra.Array(
    ...
    for i = len-bpc+1-offside:len
        cur_coeff = coeff[i]
        if cur_coeff < 0 && i <= len - offside
            L[i, i-stl+2+offside:i+1+offside] =
                cur_coeff * A.high_boundary_coefs[2, i-len+bpc+offside]
        elseif cur_coeff < 0
...
# Uniform grid case
function BandedMatrices.BandedMatrix(
    A::DerivativeOperator{T,N,true},
    len::Int = A.len,
) where {T,N}
    bpc = A.boundary_point_count
...
################################################################################
function LinearAlgebra.Array(
    A::GhostDerivativeOperator{T,E,F},
    N::Int = A.L.len,
) where {T,E,F}
    Q_l, Q_b = Array(A.Q, N)
...
Array(L::DiffEqOperatorComposition, s) =
