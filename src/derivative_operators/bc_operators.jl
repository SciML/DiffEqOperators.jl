...
struct RobinBC{T,V<:AbstractVector{T}} <: AffineBC{T}
    ...
    function RobinBC(
        l::Union{NTuple{3,T},AbstractVector{T}},
        r::Union{NTuple{3,T},AbstractVector{T}},
        dx::AbstractVector{T},
        order = 1,
    ) where {T}
        αl, βl, γl = l
...
stencil(q::AffineBC{T}, N::Int) where {T} = (
...
stencil(q::PeriodicBC{T}, N::Int) where {T} =
...
struct GeneralBC{T,L<:AbstractVector{T},R<:AbstractVector{T}} <: AffineBC{T}
    ...
    function GeneralBC(
        ...
        for i = 1:(nl-2)
            S_l[i, :] =
                [transpose(
                    calculate_weights(i, one(T), Array(one(T):convert(T, order + i))),
                ) transpose(zeros(T, Int(nl - 2 - i)))] ./ (dx^i) #am unsure if the length of the dummy_x is correct here
        end
            ...
            S_r[i, :] =
                [transpose(
                    calculate_weights(
                        i,
                        convert(T, order + i),
                        Array(one(T):convert(T, order + i)),
                    ),
                ) transpose(zeros(T, Int(nr - 2 - i)))] ./ (dx^i)
        end
        s0_l = S_l[:, 1]
        Sl = S_l[:, 2:end]
        s0_r = S_r[:, end]
    ...
    function GeneralBC(
        ...
        for i = 1:(nl-2)
            S_l[i, :] =
                [transpose(
                    calculate_weights(i, one(T), Array(one(T):convert(T, order + i))),
                ) transpose(zeros(T, Int(nl - 2 - i)))] ./ transpose(dx_l .^ i)
        end
        ...
        for i = 1:(nr-2)
            S_r[i, :] =
                ...
                ) transpose(zeros(T, Int(nr - 2 - i)))] ./ transpose(dx_r .^ i)
        end
        s0_l = S_l[:, 1]
        Sl = S_l[:, 2:end]
        s0_r = S_r[:, end]
...
NeumannBC(α::NTuple{2,T}, dx::Union{AbstractVector{T},T}, order = 1) where {T} =
    RobinBC((zero(T), one(T), α[1]), (zero(T), one(T), α[2]), dx, order)
DirichletBC(αl::T, αr::T) where {T} =
...
Neumann0BC(dx::Union{AbstractVector{T},T}, order = 1) where {T} =
...
Base.:*(Q::AffineBC, u::AbstractVector) = BoundaryPaddedVector(
    Q.a_l ⋅ u[1:length(Q.a_l)] + Q.b_l,
    Q.a_r ⋅ u[(end-length(Q.a_r)+1):end] + Q.b_r,
    u,
)
