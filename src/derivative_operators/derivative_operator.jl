...
function initialize_left_boundary!(::Type{Val{:LO}},low_boundary_coefs,stencil_coefs,BC,derivative_order,grid_step::T,
    ...
    elseif LBC == :Dirichlet0
        boundary_point_count[1] = div(stencil_length,2)
        left_Dirichlet0_BC!(Val{:LO},low_boundary_coefs,stencil_length,derivative_order,
    ...
    elseif LBC == :Dirichlet
        ...
        left_Dirichlet_BC!(Val{:LO},low_boundary_coefs,stencil_length,derivative_order,
                                              grid_step,boundary_length)
...
function initialize_right_boundary!(::Type{Val{:LO}},high_boundary_coefs,stencil_coefs,BC,derivative_order,grid_step::T,
    ...
    if RBC == :None
        ...
        #=
            ...
            function without changing the name, this trick has been applied.
        =#
        right_None_BC!(Val{:LO},high_boundary_coefs,stencil_length,derivative_order,
                               grid_step,boundary_length)
    ...
    elseif RBC == :Dirichlet0
        boundary_point_count[2] = div(stencil_length,2)
        right_Dirichlet0_BC!(Val{:LO},high_boundary_coefs,stencil_length,derivative_order,
                               grid_step,boundary_length)
        return (one(T),zero(T),zero(T))
    ...
    elseif RBC == :Dirichlet
        ...
        right_Dirichlet_BC!(Val{:LO},high_boundary_coefs,stencil_length,derivative_order,
                               grid_step,boundary_length)
        return (one(T),zero(T),ret)
...
function left_None_BC!(::Type{Val{:LO}},low_boundary_coefs,stencil_length,derivative_order,
    ...
    points               = collect(zero(T) : grid_step : (boundary_length-1)*grid_step)
    for i in 1 : boundary_point_count
        ...
        push!(low_boundary_coefs, calculate_weights(derivative_order, (i-1)*grid_step, points))
    end
    return l_diff
end
...
function right_None_BC!(::Type{Val{:LO}},high_boundary_coefs,stencil_length,derivative_order,
    ...
    for i in 1 : boundary_point_count
        # One-sided stencils require more points for same approximation order
        push!(high_boundary_coefs, calculate_weights(derivative_order, -(i-1)*grid_step, points))
    end
    return r_diff
end
...
function left_Dirichlet0_BC!(::Type{Val{:LO}},low_boundary_coefs,stencil_length,derivative_order,
    ...
    points               = collect(zero(T) : grid_step : (boundary_length-1)*grid_step)
    for i in 1 : boundary_point_count
        push!(low_boundary_coefs, calculate_weights(derivative_order, i*grid_step, points)[2:end])
    end
    return l_diff
end
...
function right_Dirichlet0_BC!(::Type{Val{:LO}},high_boundary_coefs,stencil_length,derivative_order,
    ...
    points               = reverse(collect(zero(T) : -grid_step : -(boundary_length-1)*grid_step))
    for i in 1 : boundary_point_count
        push!(high_boundary_coefs, calculate_weights(derivative_order, -i*grid_step, points)[1:end-1])
    end
    return r_diff
end
