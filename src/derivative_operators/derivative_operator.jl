...
function generate_boundary_coordinates(i, stencil_x, dx)
    len = length(stencil_x)
    T = typeof(stencil_x[1])
    _dx = dx[1:len-1]
    stencil_x[i] = zero(T)
...
function CenteredDifference{N}(derivative_order::Int,
    ...
    _low_boundary_coefs     = SVector{boundary_stencil_length, T}[convert(SVector{boundary_stencil_length, T}, calculate_weights(derivative_order, zero(T), generate_boundary_coordinates(i, boundary_stencil_x, dx))) for i in L_boundary_deriv_spots]

    low_boundary_coefs      = convert(SVector{boundary_point_count},_low_boundary_coefs)

    _high_boundary_coefs    = SVector{boundary_stencil_length, T}[convert(SVector{boundary_stencil_length, T}, reverse(calculate_weights(derivative_order, zero(T), generate_boundary_coordinates(i, boundary_stencil_x, reverse(dx))))) for i in R_boundary_deriv_spots]
