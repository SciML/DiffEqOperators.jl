...
mutable struct JacVecOperator{T,F,T1,T2,P,tType} <: DiffEqBase.AbstractDiffEqLinearOperator{T}
    f::F
    cache1::T1
    cache2::T2
    p::P
    ...
    function JacVecOperator{T}(f,p=nothing,t::Union{Nothing,Number}=nothing) where T
        ...
        new{T,typeof(f),Nothing,Nothing,P,tType}(f,nothing,nothing,nothing,nothing)
    end
    function JacVecOperator{T}(f,u::AbstractArray,p=nothing,t::Union{Nothing,Number}=nothing) where T
        ...
        p===nothing ? P = Any : P = typeof(p)
        t===nothing ? tType = Any : tType = typeof(t)
        new{T,typeof(f),typeof(cache1),typeof(cache2),P,tType}(f,cache1,cache2,p,t)
    end
...
function Base.:*(L::JacVecOperator,u::AbstractVector) = jacvec(u->L.f(u,L.p,L.t),u,u)

function LinearAlgebra.mul!(du::AbstractVector,L::JacVecOperator,u::AbstractVector)
    let p=L.p,t=L.t
        if L.cache1 === nothing
            jacvec!(du,(du,u)->L.f(du,u,p,t),u,u)
        else
            jacvec!(du,(du,u)->L.f(du,u,p,t),u,u,L.cache1,L.cache2)
        end
    end
end
