...
function compose(padded_arrays::BoundaryPaddedArray...)
    ...
    (length(padded_arrays) == N) || throw(
        ArgumentError(
            "The padded_arrays must cover every dimension - make sure that the number of padded_arrays is equal to ndims(u).",
        ),
    )
    for D in Ds
        length(setdiff(Ds, D)) < N || throw(
            ArgumentError(
                "There are multiple Arrays that extend along dimension $D - make sure every dimension has a unique extension",
            ),
        )
    end
    any(
        fill(padded_arrays[1].u, (length(padded_arrays),)) .== getfield.(padded_arrays, :u),
    ...
    ComposedBoundaryPaddedArray{
        gettype(padded_arrays[1]),
        N,
        N - 1,
        typeof(padded_arrays[1].u),
        typeof(lower[1]),
    }(
        lower,
        upper,
...
struct ComposedBoundaryPaddedArray{T,N,M,V<:AbstractArray{T,N},B<:AbstractArray{T,M}} <:
...
"""
decompose(A::ComposedBoundaryPaddedArray) = Tuple([
    BoundaryPaddedArray{gettype(A),i,ndims(A),ndims(A) - 1,typeof(lower[1])}(
        A.lower[i],
        A.upper[i],
        A.u,
    ) for i = 1:ndims(A)
...
function Base.getindex(
    Q::BoundaryPaddedArray{T,D,N,M,V,B},
    _inds::Vararg{Int,N},
) where {T,D,N,M,V,B} #supports range and colon indexing!
    inds = [_inds...]
    S = size(Q)
    dim = D
    ...
    elseif typeof(inds[dim]) == Colon
        if mapreduce(x -> typeof(x) != Colon, (|), otherinds)
            ...
            throw(
                "A colon on the extended dim is as yet incompatible with additional colons",
            )
        end
...
function Base.getindex(
    Q::ComposedBoundaryPaddedArray{T,N,M,V,B},
    inds::Vararg{Int,N},
) where {T,N,M,V,B} #as yet no support for range indexing or colon indexing
    S = size(Q)
